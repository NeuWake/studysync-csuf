...
import { Loader2, Upload, FileText, Download, Trash2, Eye, X, Image as ImageIcon } from "lucide-react";
...
export function AssignmentDocumentsInline({ userAssignmentId }: Props) {
  const { user } = useAuth();
  ...
  return (
    <div className="mt-3 pt-3 border-t space-y-2" onClick={(e) => e.stopPropagation()}>
      ...
      {isLoading ? (
        ...
        <p className="text-xs text-muted-foreground py-2">No files attached yet.</p>
      ) : (
          ...
          {docs.map((d: any) => {
            ...
            return (
              <div key={d.id} className="flex items-center gap-2 p-2 rounded-md border bg-muted/30">
                ...
                {previewable && (
                  <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => openPreview(d)} title="Preview">
                    <Eye className="h-3.5 w-3.5" />
                  </Button>
                )}
                <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => downloadDoc(d)} title="Download">
